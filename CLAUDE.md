...
workflow rebuilds `_site` and deploys. **Every deploy: change `SHELL_REV` (www/sw.js) so installed
clients re-fetch the shell** — normally in lockstep with `APP_VERSION` (www/index.html). When
Steven wants the on-screen chip held at a value across several ships (as with the multi-batch
v0.1.3), keep `APP_VERSION` fixed and give `SHELL_REV` a suffix (`v0.1.3-gt`, `v0.1.3-ux`, …) so the
SW cache still busts. A shell change with an UNCHANGED `SHELL_REV` will NOT reach installed phones
(the SW keeps serving the cached shell) — this bit us once in v0.1.3-ux (panel came up empty).
...
| Version | What | Where |
|---|---|---|
| **v0.1.4** | 🎚 Bottom-sheet UX refactor (sister-synced with AuraGold v44.0). Layers panel is now a **draggable bottom-sheet** (peek/half/full · drag handle · scrolls · map stays visible & interactive above); the top-left **Signal dropdown moved INTO the sheet** as a segmented selector in *Geology & geophysics*; the Menu is rebuilt as **collapsible accordions** (📍 Site info · 🎨 Legend · 🔬 Technologies · 📚 Research · ℹ️ About) — Technologies = 5 tight cards (ERT/GPR/Mag/Rad/Hillshade), Research = 3 pointers → GitHub blob. Placement locked to the shared 5-cluster rule (zoom-only TL · Layers-only TR · pill+chip TC · 3D/Menu/Locate BR · green/red CTAs BL); no orphan controls. Sheet is `position:absolute` + **% transforms + ratio drag** (no `window.innerHeight`, DPR-safe). | `www/index.html` · `www/sw.js` |
| **v0.1.3** | 🧩 Three batches under one chip (latest SW cache-rev `v0.1.3-ux`). **(c) AuraGold UX port** (sisters): ported AuraGold's unified **Layers panel** (top-right) — grouped rows, each with a checkbox + swatch + **live opacity slider** + %, backed by `localStorage.ao_ly` (per-layer {on,op}, persists across reload). **Toggle wiring rebuilt** on that pattern (Steven flagged the old drawer toggles as broken). One pane per layer → `setPaneOpacity`. Button placement mirrors AuraGold: zoom top-left, Layers top-right, title pill + status chip top-centre, a **Signal** variant dropdown (switches the active geophysics raster), bottom-right FAB stack (🌐 3D placeholder / ☰ Menu / 📍 Locate), bottom-left CTAs (green Start-survey timer, red Log-a-find GPS pin → `localStorage.ao_myfinds`). Menu (right slide-in) holds legend + site-info + about. Kept the opal-teal accent (sisters, not clones). **(a) Opal-formation model** in Site info: clay low-points pool silica-rich water → mudstone→ironstone → thickest opal immediately above the clay; ★ KEY stratigraphy annotation + "How opal forms here" (cited to operator notes) + future-model target = clay-surface TOPOGRAPHY, not depth-to-clay. **(b) KML v2 ground-truth rebuild**: re-ingested the full re-exported KML → **18 opal-find pins** (was 10), **12 field photos** in pin popups + a full-screen lightbox (bundled offline), the **"area mined to 1m above 1st clay layer" polygon** (muted/dashed), 8 trenches (trench/potential/path), claim+fault from KML. New **"main patch"** note (dense cluster ~50 m E of the western finds, ~258 m = ~2 m shallower). **Geology correction: false/intermediate clay layers are productive too** (smaller local dams) → ERT wins (maps ALL clay contrasts). Esri imagery native ceiling capped at z17 (z18+ blank over Opalton). | `www/index.html` · `data/photos/opalton/` · `research/` · `analysis_v2.md` |
...
ironstone layers (bottom usually best) → sandstone → basal **clay layer**. Steven's ground truth:
best gem/red opal clusters in a ~40×40 m box **west of the fault** at ~4 m depth; **fault-adjacent
finds run potchy** (fractures carry water but make patch, not colour). Depth stratification is real
...
ironstone (→ magnetics), weathering/clay (→ radiometrics K/Th/U), structure (→ faults/lineaments),
historical activity (→ tenement/claim density).
