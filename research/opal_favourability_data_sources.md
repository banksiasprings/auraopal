...
- **GPR** fires down through clean, resistive **sandstone** (radar-friendly), **images the ironstone target**, and only then gets absorbed at the **basal clay beneath it** — which is fine, because the clay floor is *below* what you care about. The target is imaged before the signal dies.
- GPR only loses when clay **caps** the sandstone above the target (the minority case) — then the pulse dies before reaching the ironstone.
- **Electrical methods (ERT / EM)** still key beautifully off the clay-vs-ironstone contrast and don't care about a cap, so they're the best fallback where capping clay is present or unknown.
...
### 1. Ground-penetrating radar (GPR) — the top pick for typical uncapped ground
...
### 3. Electrical resistivity tomography (ERT) — the fallback for capped/unknown ground
